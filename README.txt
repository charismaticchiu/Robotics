...
copyright on those changes to MobileRobots, contact 
...
Note: If you use a different version, you must rebuild the ARIA 
libraries, but MobileRobots only supports Visual Studio .NET 2003,
and only versions 7.1 and later are probably able to correctly build
ARIA without adding workarounds to the code.
...
were built with G++ 3.4. You must use G++ 3.4 or a compatible version to
...
You may need to install it yourself on your own system if not already present. 
