...
AREXPORT bool ArRobotConnector::parseArgs(ArArgumentParser *parser)
{
  ...
  if (parser->checkArgument("-ris") ||
      parser->checkArgument("-remoteIsSim"))
    myRemoteIsSim = true;

  if (parser->checkArgument("-rlpr") ||
      parser->checkArgument("-robotLogPacketsReceived"))
    myRobotLogPacketsReceived = true;
  
  if (parser->checkArgument("-rlps") ||
      parser->checkArgument("-robotLogPacketsSent"))
    myRobotLogPacketsSent = true;

  if (parser->checkArgument("-rlmr") ||
      parser->checkArgument("-robotLogMovementReceived"))
    myRobotLogMovementReceived = true;

  if (parser->checkArgument("-rlms") ||
      parser->checkArgument("-robotLogMovementSent"))
    myRobotLogMovementSent = true;
  
  if (parser->checkArgument("-rlvr") ||
      parser->checkArgument("-robotLogVelocitiesReceived"))
    myRobotLogVelocitiesReceived = true;
  
  if (parser->checkArgument("-rla") ||
      parser->checkArgument("-robotLogActions"))
    myRobotLogActions = true;

  if (!parser->checkParameterArgumentString("-rh", 
					    &myRemoteHost) ||
      !parser->checkParameterArgumentString("-remoteHost", 
					    &myRemoteHost) ||
