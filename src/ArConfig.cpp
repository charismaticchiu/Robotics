        ...
        (curSection->findParam(arg.getName()) != NULL)) 
    {
      if (strcasecmp(curSection->getName(), section->getName()) != 0) {
        ArLog::log(ArLog::Verbose, 
                  ...
                  myLogPrefix.c_str(),
		              arg.getName(), curSection->getName(), section->getName());
        myDuplicateParams = true;
      }
      else {
        ArLog::log(ArLog::Verbose, 
                  "%sParameter %s already exists in section %s",
                  myLogPrefix.c_str(),
		              arg.getName(), section->getName());
      }
    }
  } // end for each section
				      ...
				      size_t errorBufferLen)
{
       ...
       sectionIt++)
  {
	...
	else if (param->getType() == ArConfigArg::DOUBLE)
	{
	  ...
	  {
      IFDEBUG(ArLog::log(ArLog::Verbose, "%sSet parameter '%s' to '%.10f'",
	...
	else if (param->getType() == ArConfigArg::BOOL)
	{
	  ...
	  {
      IFDEBUG(ArLog::log(ArLog::Verbose, "%sSet parameter '%s' to %s",
	...
	else if (param->getType() == ArConfigArg::STRING)
	{
	  ...
	  {
      IFDEBUG(ArLog::log(ArLog::Verbose, "%sSet parameter string '%s' to '%s'",
                          myLogPrefix.c_str(),
	...
	else if (param->getType() == ArConfigArg::FUNCTOR)
	{
	  ...
	  {
      IFDEBUG(ArLog::log(ArLog::Verbose, "%sSet arg '%s' with '%s'",
				     ...
				     bool writePriorities)
{
  ...
  if (section->getComment() != NULL && strlen(section->getComment()) > 0)
  {
    ...
    descr.setQuiet(myIsQuiet);
    descr.addPlain(section->getComment());
  ...
  for (paramIt = params->begin(); paramIt != params->end(); paramIt++)
  {
    ...
    if (param->getType() == ArConfigArg::FUNCTOR)
    {
        ...
        strlen(param->getDescription()) > 0)
      {
        ArArgumentBuilder descr;
        descr.setQuiet(myIsQuiet);
        descr.addPlain(param->getDescription());
        for (unsigned int i = 0; i < descr.getArgc(); i++)
        {
          // see if we're over, if we are write this line out and start
          // the next one
