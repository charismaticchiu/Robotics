 ...
 * camera remotely. 
 *
 * To use, first run SAVserver. Then run serverDemo, or guiServer from ARNL or
 * guiServerSonar from SONARNL on the robot's onboard computer. Or run your own server
 * program that includes ArServerHandlerCamera.  Then from another host, 
 * run ptzCameraClientExample with the -host argument.  For example, if the hostname of
 * the robot's onboard computer is "robot", run:
